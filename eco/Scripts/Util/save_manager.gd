...
var FacadesPaths: Dictionary = {
	"source": "res://Saves/facades_paths.json"
}

var DataPaths: Dictionary = {
	"source": "res://Saves/data_paths.json"
}
...
func load_from_json() -> void:
	load_dictionary_from_json(FacadesPaths)
	load_dictionary_from_json(DataPaths)
...
func load_dictionary_from_json(dict: Dictionary) -> Dictionary:
	if not FileAccess.file_exists(dict["source"]):
		return {"source": dict["source"]}
	var file = FileAccess.open(dict["source"], FileAccess.READ)
