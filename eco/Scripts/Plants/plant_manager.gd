...
var plants_backfill_queue: Array[PlantData] = []
var backfill_queue_amt: int
...
func _process(_delta: float) -> void:
	...
	while backfill_queue_amt > 0:
		...
		# Process a plant
		var plant = plants_backfill_queue.pop_back()
...
func next_room() -> void:
	...
	cur_region_cons = compile_conditions(cur_room.get_region_conditions(), cur_update_time)
...
func compile_conditions(region_cons: Dictionary, end_time: float) -> Dictionary:
	...
	for i in range(num_ranges - 1):
		...
		compile_cons[num_ticks] = MathUtil.vec3_from_vec4(region_cons[timestamps[i]])
...
func room_ready(r: RoomData) -> void:
	var new_plants = r.get_plants_data()
	plants_backfill_queue.append(new_plants)
	var num_plants = new_plants.size()
	plants_in_room_queues.append(num_plants)
	backfill_queue_amt += num_plants
	rooms.append(r)
